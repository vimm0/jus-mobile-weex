          ...
          <template v-if="isSubscriptionActive && !subscriptionEnds && !problemInSubscription">
            <template v-if="subscription[0].status === 'trialing'">
              <p><span
                ...
                }} Days left </span> on {{ $options.filters.capitalizeText(subscription[0].status) }} Subscription</p>
            </template>
            <h2 class="title is-4">✓ {{ $options.filters.capitalizeText(subscription[0].status) }} Subscription</h2>
            ...
            <template v-if="noSubscription || subscriptionEnds">
              <div class="field is-horizontal">
                ...
                <div class="field-body">
                  <b-dropdown v-model="isPlanPublic">
                    ...
                    <b-dropdown-item v-for="plan in plans" :key="plan.id">
                      <div class="media" @click="planDropDownSelected(plan)">
                        <div class="media-content">
                          <small>Plan Name: {{ $options.filters.capitalizeText(plan.name) }}</small>
                          <br>
              ...
              <div class="field is-horizontal">
                ...
                <div class="field-body">
                  <b-dropdown v-model="isCardPublic">
                    <button class="button is-small" type="button" slot="trigger">
                      <template v-if="isCardPublic">
                        <span>None</span>
                      </template>
                      <template v-else>
                        <span> {{ selectedCard.fingerprint }} </span>
                      </template>
                    </button>
                    <b-dropdown-item v-for="card in cards" :key="card.id">
                      <div class="media" @click="cardDropDownSelected(card)">
                        <div class="media-content">
                          <!--<h1>Name: {{ $options.filters.capitalizeText(card.customer) }}</h1>-->
                          <p>Card Type: {{ $options.filters.capitalizeText(card.funding) }}</p>
                          <b>****{{ card.last4 }} Ending {{card.exp_month}}/{{card.exp_year}}</b>
                        </div>
                      </div>
                    </b-dropdown-item>
                  </b-dropdown>
              ...
              <div class="field is-horizontal">
                ...
                <div class="field-body">
                  <button @click="subscribe" class="button is-success is-outlined">Start Subscription</button>
                </div>
              </div>
            </template>
          </template>
