...
<template>
  <div class="payment-card-registration">
    <!-- <b-collapse class="card"> -->
      ...
      <template v-else> -->
        <div class="card-content">
          <!-- <b-notification class="is-danger" v-if="error">
          ...
          <button v-on:click="purchase" class="custom-button button is-success is-outlined">Add A New Card</button>
          <hr v-if="cards.length > 0">
          <h2 class="title is-5" v-if="cards.length > 0">All Cards</h2> -->
          <!-- <div v-for="customer_obj in customer">
              ...
              <section>
                <b-collapse class="card" :open="isOpen">
                  ...
                  <div class="card-content">
                    ...
                    <template v-if="customer_obj.default_source !== card.stripe_id">
                      <button class="button is-small is-primary" @click="changeDefaultCard(card.id, card.stripe_id)">
                        Make Default
                      </button>
                      <button class="button is-small is-danger" @click="removeCard(card.id)">Remove</button>
                    </template>
                  </div>
                </b-collapse>
              </section>
    ...
    methods: {
      ...
      removeCard (id) {
        ...
        if (confirm('Are you sure you want to remove this card?')) {
          global.axios.delete(`card/${id}`).then(response => {
            console.log(response)
            this.$router.go()
          }).catch(error => {
            console.log(error.response.data.response)
...
//          }
        ...
        global.axios.get('customer/').then(response => {
          console.log(response)
